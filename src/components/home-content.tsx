...
export default function HomeContent() {
  ...
  return (
    <div className="relative flex flex-1 flex-col items-center justify-center overflow-hidden bg-zinc-50 px-6 py-24 font-sans dark:bg-black">
      ...
      {/* Ambient background glow, spreading out from the Q rather than
          confined to a hard-edged square. Two oversized (90vmax/70vmax —
          well past any screen edge at normal zoom), heavily blurred,
          low-opacity circles, each fading smoothly to fully transparent
          well before its own edge, so there's no boundary left to read as a
          shape — the same way real ambient light falls off. Two overlapping
          colours — violet/indigo for the purple half, a deep teal/marine
          blue (Tailwind's cyan-950/sky-900 range) for the other — offset
          slightly from each other so they blend into one soft wash rather
          than a flat single-colour glow. `mix-blend-screen` lets the two
          colours actually combine where they overlap instead of one flatly
          occluding the other. Sits below the Q glyph and the card in DOM
          order, so both still read clearly on top of it. */}
      ...
      {/* Large decorative "Q" watermark behind the card — Playfair Display
          (loaded in src/app/layout.tsx), sized to roughly frame the content.
          Purely decorative: aria-hidden, no pointer events, doesn't affect
          layout flow of the real content sitting above it (z-10). Carries an
          indigo tint and enough opacity to actually read as part of the
          glow rather than being hardly visible. Blurred the glyph itself:
          the part of the Q sitting behind the glass card was already soft
          (thanks to the card's own backdrop-blur), but the tail poking out
          below the card rendered crisp against the plain background, so it
          read as a stray sharp shape rather than part of the same glow. A
      ...
      <main className="relative z-10 flex w-full max-w-md flex-col items-center gap-8 rounded-3xl border border-black/[.06] bg-white/60 px-8 py-10 text-center shadow-[0_1px_3px_rgba(0,0,0,0.06)] backdrop-blur-sm dark:border-white/[.08] dark:bg-white/[.04]">
        ...
        {/* Still discreet — muted, unlabeled beyond a single lowercase word,
            ...
            flow itself, discreet but easier to actually find. The old
            placement — a fixed corner of the whole viewport, `text-zinc-300`
            (nearly
